...
import { FaChartLine, FaClipboardList, FaMoneyBillWave, FaRegBuilding, FaSolarPanel, FaUser, FaUserShield, FaUsers, FaWrench, FaBell, FaExchangeAlt, FaFileInvoice } from 'react-icons/fa'
    ...
    <div className="dashboard-page">
      ...
      <div className="dashboard-table-filters" style={{ marginBottom: 16 }}>
        <div className="dashboard-search">
          <input className="crm-input" placeholder="Search by name, number, email..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
        ...
        <table className="crm-table">
          ...
          <tbody>
            {filteredUsers.map(u => {
              ...
              return (
                <tr key={u._id}>
                  <td style={{ fontSize: 20, display:'flex', justifyContent:'center' }}>{(() => { const Icon = ROLE_ICON_MAP[u.role] || FaUser; return <Icon /> })()}</td>
                  ...
                  <td>
                    {!isSelf && (
                      <div className="dashboard-inline-actions" style={{ gap: 6 }}>
                        {isPending && <button className="btn btn-success btn-sm" disabled={actioningId === u._id} onClick={() => handleApprove(u._id)}>Approve</button>}
                        {(isPending || !isRejected) && <button className="btn btn-danger btn-sm" disabled={actioningId === u._id} onClick={() => handleReject(u._id)}>Reject</button>}
                        {isRejected && <button className="btn btn-success btn-sm" disabled={actioningId === u._id} onClick={() => handleApprove(u._id)}>Re-Approve</button>}
                        <button className="btn btn-danger btn-sm" disabled={actioningId === u._id} onClick={() => handleDelete(u._id)}>Delete</button>
                      </div>
                    )}
                  </td>
                </tr>
    ...
    <div className="dashboard-page" style={{ maxWidth: 600 }}>
      ...
      <div className="crm-card" style={{ marginBottom: 16 }}>
        <div style={{ display: 'flex', gap: 16, alignItems: 'center', marginBottom: 20 }}>
          <div style={{ width: 64, height: 64, borderRadius: 20, background: 'linear-gradient(135deg,rgba(245,158,11,.2),rgba(249,115,22,.1))', border: '1px solid rgba(245,158,11,.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28 }}>{(() => { const Icon = ROLE_ICON_MAP[user?.role] || FaUser; return <Icon /> })()}</div>
          <div>
